...
$T_w$ is well-defined.  Insert
the letter $r$ into a permutation of $S_{r-1}$ and prove
\[
...
$T_ie_+=ve_+$ and $T_ie_-=-v^{-1}e_-$.  Under the fundamental Hecke
action from the preceding part, identify their images with the quantum
symmetric and exterior state spaces.
\end{exercise}
...
with $p(0)=1$.  Euler's partition generating function is
\[
...
even unimodular lattice, use consistency of the $S$ and $T$ laws to prove
that its dimension $d$ is divisible by eight and that its theta series is a
modular form of weight $d/2$.  Construct the $E_8$ lattice as
\[
...
\]
for $|z|<\|A\|^{-1}$.  Prove that the determinant is entire and that its
zeros are the reciprocals of the negatives of the nonzero eigenvalues of
$A$.  Construct the completed exterior powers of $H$ and deduce
\[
\det(I+zA)=\sum_{m=0}^{\infty}z^m\Tr(\Lambda^mA).
\]
\end{exercise}
