...
\begin{exercise}[Electromagnetic fields, differential forms, and Cartan calculus]
For a particle moving on a configuration manifold $M$, a scalar potential
is a function, a work or gauge potential is a one-form, and its field
strength is the exterior derivative of that one-form.  A vector field $X$
describes a continuous motion of the particle or medium and therefore
transports all of these quantities.

A cotangent space is $T_p^*M=(T_pM)^*$, and $T^*M$ is the union of these
spaces over $p\in M$.
...
\]
For a gauge potential $A\in\Omega^1(M)$ with field strength $F=dA$, deduce
\[
\mathcal L_XA=\iota_XF+d(\iota_XA),
\]
separating the field contribution from an exact gauge term.
\end{exercise}
...
\begin{exercise}[Classical angular momentum and coadjoint orbits]
The angular momentum of a rigid body has fixed magnitude under pure
rotation, so its possible values lie on a sphere rather than in a vector
space.  More generally, fixing the conserved quantities generated by a
continuous symmetry group produces a coadjoint orbit.
...
For $G=SO(3)$, identify $\mathfrak{so}(3)$ and its dual with $\mathbb R^3$
using the cross product and Euclidean pairing.  Show that the nonzero
coadjoint orbit through angular momentum $\ell$ is the sphere
$|L|=|\ell|$, that
\[
\omega_\ell(X\times\ell,Y\times\ell)
=\ell\cdot(X\times Y),
\]
and that its total symplectic area is $4\pi|\ell|$.
\end{exercise}
...
\begin{exercise}[Factorized spin interactions and Poisson--Lie groups]
In a classical integrable spin chain, composing two subsystems must preserve
their Poisson brackets, while consistency of three successive two-body
interactions must be independent of their ordering.  These requirements
lead respectively to Poisson--Lie multiplication and the classical
...
distinguish the case $\Omega\ne0$.  Identify the three terms of
$\operatorname{CYB}(r)$ with the three pairwise interactions in the two
infinitesimal factorizations of a three-spin process.
\end{exercise}
...
\begin{exercise}[Quasiparticle exchange and universal $R$-matrices]
When two excitations scatter in a one-dimensional integrable medium, their
internal state can be transformed while their order is exchanged.
Requiring the two factorizations of a three-excitation collision to give
the same final state is the Yang--Baxter consistency condition.
...
\begin{exercise}[Anyon worldlines and Temperley--Lieb projectors]
In a planar gapped medium, creation, propagation, and annihilation of paired
quasiparticles draw nonintersecting worldline diagrams.  Concatenating
experiments stacks diagrams, placing independent experiments side by side
tensors them, and an unobserved closed loop contributes a scalar amplitude
$d$.
...
The pendulum is the simplest oscillator for which the harmonic
approximation misses an amplitude-dependent period.  Its exact return time
already forces an elliptic special function.
...
\begin{exercise}[Particles on a torus and double-affine braid groups]
Consider indistinguishable particles moving without collision on a
two-dimensional periodic sample.  Their histories record adjacent
exchanges and independent windings around the two noncontractible cycles;
a full cycle commutator may retain a central phase.
...
\begin{exercise}[Programmable Askey--Wilson spectrum]
A finite programmable network can be tuned so that its normal modes are
governed by a self-adjoint $q$-difference operator.  Driving the network by
the coordinate observable then turns the polynomial recurrence into
...
\begin{exercise}[Local lattice weights and elliptic gamma functions]
In an exactly solvable two-dimensional lattice model, a local Boltzmann
weight depends on a rapidity variable, two independent periodic scales,
and an inversion relation.  Model the required two shift laws and
reciprocal pairing by the following elliptic gamma function.
