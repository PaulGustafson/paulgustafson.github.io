...
For $p=1,2$, define $L^p(\R^n)$ as the completion of the continuous
compactly supported functions $C_c(\R^n)$ in
\[
\|f\|_p=\left(\int_{\R^n}|f(x)|^p\,dx\right)^{1/p}.
\]
Define $L^2(\T)$ as the completion of $C(\T)$ for the inner product
$\langle f,g\rangle=(2\pi)^{-1}\int_0^{2\pi}\overline{f(x)}g(x)\,dx$.
A complete inner-product space is a \emph{Hilbert space}, so $L^2(\T)$ and
$L^2(\R^n)$ are Hilbert spaces by construction. Measure theory will later
identify these completions with measurable functions modulo almost-everywhere
equality.
\end{definition}
...
\begin{supplied}[Unbounded Hamiltonians]
Physical Hamiltonians such as the Coulomb and Schr\"odinger operators are
generally unbounded. We take as analytic input the theory of densely defined
closed and self-adjoint operators, their domains, the unbounded spectral
theorem and functional calculus, and Stone's theorem. All claims below about
such Hamiltonians are understood on their invariant domains; the later
bounded spectral theorem does not replace this input.
\end{supplied}
...
For Lebesgue measure on $\R^n$ or normalized arc measure on $\T$, this
measurable-function model is canonically isometric to the completions used
in Fourier analysis.
\end{definition}
...
a Hilbert space, prove the asserted identification with the earlier
completions, and identify the dual of $L^p$ for $1<p<\infty$ under the
...
expectations, and exponentials are interpreted using the supplied
unbounded-operator theory and their natural domains.
\end{definition}
