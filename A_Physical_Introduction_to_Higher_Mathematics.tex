...
\begin{exercise}[Nonlocal games force operator-space norms]
...
\begin{exercise}[The Khot--Vishnoi separation]
Let $n=2^\ell$ and let $H\subset\{0,1\}^n$ be the Hadamard code
$h_s(t)=s\cdot t$ for $s,t\in\mathbb F_2^\ell$. The questions are cosets of
$H$. The referee chooses $[x]$ uniformly and a noise string $z$ whose bits
are independent Bernoulli variables with parameter $\eta$, and sends $[x]$
to Alice and $[x+z]$ to Bob. They answer $a\in[x]$ and $b\in[x+z]$ and win
when $a+b=z$ over $\mathbb F_2^n$.

For $\eta=1/2-1/\log n$ and large $n$, take as input the
Bonami--Beckner classical estimate $\omega(G_{\rm KV})\leq C/n$. Verify the
explicit strategy using a maximally entangled state of local dimension $n$
and obtain $\omega^*(G_{\rm KV})\geq c/(\log n)^2$. Deduce
\[
\frac{\|\widehat G_{\rm KV}\|_{\min}}
     {\|\widehat G_{\rm KV}\|_\varepsilon}
=\frac{\omega^*(G_{\rm KV})}{\omega(G_{\rm KV})}
\geq c'\frac{n}{(\log n)^2}.
\]
Explain why this unbounded physical separation rules out replacing the
minimal norm by the injective norm with a dimension-independent constant on
these spaces, and is the reason the matrix levels of operator spaces are
essential. Consult
\href{https://doi.org/10.4086/toc.2012.v008a027}
{Buhrman--Regev--Scarpa--de Wolf, \emph{Theory of Computing} \textbf{8}
(2012)} for the estimates.
\end{exercise}

\begin{exercise}[Grothendieck inequalities for correlation games]
State and prove the finite-dimensional real Grothendieck inequality for
bilinear forms on $\ell_\infty^m\times\ell_\infty^n$. Using Tsirelson's
vector representation, deduce the dimension-independent bound
$\beta^*(G)\leq K_G^{\mathbb R}\beta(G)$ for two-player XOR games. State a
noncommutative or operator-space Grothendieck inequality with its correct
auxiliary norms and hypotheses. Explain why the XOR conclusion does not
give a universal bound on $\omega^*(G)/\omega(G)$ for arbitrary
multi-output games, and reconcile this with the Khot--Vishnoi exercise.
\end{exercise}
