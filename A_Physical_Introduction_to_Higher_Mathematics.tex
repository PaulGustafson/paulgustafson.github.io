...
\begin{definition}[Principal and associated bundles]
A \emph{smooth principal $G$-bundle} is a smooth map $\pi:P\to M$ with a
free right $G$-action, transitive on each fiber, such that every point of
$M$ has a neighborhood $U$ with an equivariant diffeomorphism
$\pi^{-1}(U)\cong U\times G$. For a representation $G\to GL(V)$, the
\emph{associated vector bundle} is
$P\times_GV=(P\times V)/((pg,v)\sim(p,gv))$.
\end{definition}

\begin{exercise}[Bundles from gauge transition functions]
Show that local trivializations of $P$ determine smooth maps
$g_{ij}:U_i\cap U_j\to G$ satisfying $g_{ij}g_{jk}=g_{ik}$, and conversely
construct $P$ from such data. Derive the transition functions of
$P\times_GV$ and characterize a global gauge choice as a global section of
$P$.
\end{exercise}
...
\begin{definition}[Topological bundles and sections]
...
spaces as fibers and linear transition maps. A topological principal
$G$-bundle is the continuous analogue of the smooth principal bundle used
for gauge fields. A \emph{section} is a map $s:X\to E$ with
$\pi s=\id_X$.
\end{definition}
