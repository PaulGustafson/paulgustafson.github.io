...
$:J_{x,p}J_{y,q}:=J_{x,p}J_{y,q}$ for $p<0$ and
$J_{y,q}J_{x,p}$ for $p\geq0$.  Contract the two current factors with
$\mathcal C_{\mathfrak g}$ and define the Sugawara operators, written
symbolically as
\[
 L_n=\frac1{2(k+h^\vee)}
 \sum_{m\in\Z}\sum_a:J^a_{n-m}J_{a,m}: .
\]
