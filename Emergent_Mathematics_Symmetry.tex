...
\begin{exercise}[$C^*$-algebras, states, and projective symmetry]
...
Show that the direct sum over all states is faithful.  Define the projective
state space $\mathbb P(\mathcal H)$ as the one-dimensional closed subspaces
of a Hilbert space and its transition probability by
\[
 p([x],[y])=\frac{\abs{\inner{x}{y}}^2}{\norm{x}^2\norm{y}^2}.
\]
Define $U(A)=\{u:u^*u=uu^*=1\}$ and prove that every representation makes
$U(A)$ act on $\mathbb P(\mathcal H)$ preserving $p$.  For $h=h^*\in A$,
identify its geometric symmetry group with
$U(A)_h=\{u:uhu^*=h\}$ and prove that every $\pi(u)$ in this group commutes
with every polynomial in $\pi(h)$.
\end{exercise}
...
\]
Write $\mathcal B(\mathcal H)$ for the $C^*$-algebra of bounded operators and
prove that $uP(B)u^*=P(B)$ for every
$u\in U(\mathcal B(\mathcal H))_H$, so the spectral resolution is equivariant
for the geometric symmetry above.  Prove the bounded Borel functional
calculus, spectral mapping, and
...
on a compact group $K$ to total mass one.  Let $K\times K$ act on $K$ by
