...
$A\simeq C(\widehat A)$.  For a bounded self-adjoint operator $H$, apply this
...
\begin{exercise}[Locally compact abelian groups and Haar measure]
Let $G$ be a locally compact abelian group.  Construct a nonzero positive
translation-invariant functional on $C_c(G)$ by the covering-number argument,
construct its regular Borel measure by outer approximation, and prove the
Riesz--Markov representation and uniqueness needed to obtain Haar measure
$\dd x$, unique up to scale.  Define convolution and involution by
\[
 (f*g)(x)=\int_G f(y)g(x-y)\dd y,
 \qquad f^*(x)=\overline{f(-x)},
\]
and prove Young's $L^1*L^p$ inequality and associativity.
\end{exercise}
...
Normalize Haar measure on a compact group $K$ to total mass one.  Average an
...
Define a connected complex reductive group $G$, a maximal torus $T$, its
...
In the group ring of the weight lattice define
...
\]
after defining exterior powers as alternating tensor quotients and the
determinant as the scalar by which an endomorphism acts on the highest
exterior power.  Derive the tableaux expansion, Cauchy identity, and
...
Define tangent and cotangent bundles, differential forms, pullback, wedge
product, and the exterior derivative.  Define symplectic manifolds and
